...
//ページネーション　１２件取得する
 ...
 // -1などのページ数として不正な値を渡された場合の対策
    ...
    $hoge = implode(',', $like_data);
      ...
      $sql = 'SELECT `feeds`.*, `users`.`user_id`  as name, `relations`.`relation_name`, `events`.`event_name`, `ages`.`generation`, `jobs`.`job_name` FROM `feeds` LEFT JOIN `users` ON `feeds`.`user_id` = `users`.id  LEFT JOIN `relations` ON `relations`.`id` = `relation_id` LEFT JOIN `events` ON `events`.`id`= `event_id` LEFT JOIN `ages` ON `ages`.`id` = `feeds`.`age_id` LEFT JOIN `jobs` ON `jobs`.`id` = `feeds`.`job_id` WHERE `feeds`.`id` IN ('.$hoge.') ORDER BY DESC LIMIT '. CONTENT_PER_PAGE .' OFFSET ' . $start;
